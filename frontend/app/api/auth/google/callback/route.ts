...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const result=await authRequest('finish',{state,browserToken,code},request.headers.get('CF-Connecting-IP')||undefined);if(!/^[a-f0-9]{64}$/.test(result.token))throw new Error('Invalid session');
  response=localRedirect('/operations');
  response.cookies.set(SESSION_HINT,'1',{sameSite:'lax',secure:request.nextUrl.protocol==='https:'||request.headers.get('x-forwarded-proto')==='https',path:'/',maxAge:7*86400});
