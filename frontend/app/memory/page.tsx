  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setDigest(digestMessages(msgs.map((m) => ({ role: m.role, content: m.content, createdAt: m.createdAt }))));
        setSource('local');
      } catch {}
    })();
    return () => { on = false; };
    ...
    <div className="py-6">
      ...
      {!memoryEnabled ? (
        <div className="p-4 bg-yellow-950 border border-yellow-700 rounded-xl text-sm">
          Memory is paused — turn it on in Settings to save conversations and learn topics.
          Nothing new is being stored.
        </div>
      ) : !digest ? (
        <p className="text-gray-400">Nothing learned yet. Go Active and talk.</p>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center gap-6">
            <div>
              <AnimatedCounter value={digest.totalMessages} className="text-3xl font-bold text-green-400" />
              <div className="text-xs text-gray-500">messages</div>
            </div>
            <div>
              <AnimatedCounter value={digest.activeDays} className="text-3xl font-bold text-green-400" />
          ...
          {digest.routineNotes.length > 0 && (
            <div className="p-4 bg-gray-900 border border-gray-700 rounded-xl">
              ...
              {digest.routineNotes.map((n) => <div key={n} className="text-sm text-gray-300">· {n}</div>)}
            </div>
          )}
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {digest.topics.map((t) => (
                <span key={t.topic} className="px-3 py-1 bg-gray-900 border border-gray-700 rounded-full text-sm">
                  #{t.topic} ×{t.count}
                </span>
              ))}
            </div>
          </div>
          <a href="/memory/timeline" className="inline-block underline text-sm">View timeline →</a>
        </div>
      )}
    </div>
  );
