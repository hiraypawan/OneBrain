...
export default function Timeline() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (on) setDigest(digestMessages(msgs.map((m) => ({ role: m.role, content: m.content, createdAt: m.createdAt }))));
      } catch {}
    })();
    return () => { on = false; };
  }, []);
  ...
  if (!digest || digest.perDay.length === 0) {
    return <div className="py-6"><h1 className="text-xl font-bold">Memory timeline</h1><p className="text-gray-400">Daily activity will appear here after you chat.</p></div>;
  }
      ...
      <div className="space-y-2">
        {digest.perDay.map((d) => (
          <div key={d.day} className="flex items-center gap-3">
            <span className="text-xs text-gray-400 w-24">{new Date(d.day).toLocaleDateString()}</span>
            <div className="flex-1 h-4 bg-gray-900 rounded overflow-hidden">
              <div className="h-4 bg-green-600" style={{ width: `${Math.max(4, (d.count / max) * 100)}%` }} />
            </div>
