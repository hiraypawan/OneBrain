...
- **Two-screen UI rework → five-tab shell (2026-09-16):** Today stayed the first-use capture/answer screen, and the navigation became **Today · Voice · Track · Space · You** (`components/AppHeader.tsx`), reusing the existing mobile bottom-bar CSS with 44 px targets, `aria-current="page"` and prefix-based highlighting so old URLs still light the right tab. `/voice` is the full-screen listening surface, `/track` is the logged-life tab, `/you` is the preferences summary. Your space is now four grouped sections behind one search box instead of a fifteen-tile wall, and retired `?panel=` routes redirect. Decorative demo widgets (`gooey-nav`, `gravity-letters`, `fluid-orb`, `notification-bell`) were deleted with their styles. **Finished 2026-09-17:** the 1,541-line `components/workspace/NeuralWorkspace.tsx` is gone and Today is the calm brief (`components/today/` — a 494-line page plus five small surfaces — and `lib/today-brief.ts`, unit-tested): greeting, date, only facts true today, one capture box, the mic, next-up and the last four saved records. Search, the context map and the receipt log with undo moved to Your space → **Notes & activity** (`?panel=notes`), sharing `components/workspace/ItemSheet.tsx` with Today so no form was forked; `?panel=notes`, `canvas` and `activity` all resolve, so nothing that was ever printed or spoken died. `/active` now opens the Voice surface. CSS: `app/shell.css` was folded into `app/product.css`, `globals.css` kept only base + tokens + Tailwind (1,570 → 73 lines), and `scripts/css-consolidate.mjs` deleted 122 dead rules and re-anchored the rest onto the surface that now renders each widget — 4,925 lines across three app stylesheets became 4,615 across two. The first mechanical version of that rewrite mapped every retired wrapper onto Today’s root and gave the brief a 260px empty grid column plus a −48px header bleed: horizontal overflow at 320/390/768/1440px, caught by the browser suite and invisible in the diff, which is why the script decides per rule instead of per prefix. The `theme` preference gained a real control (Your space → Advanced, **Dark** or **Light (beta)**) after having neither a switch nor a consumer; nothing claims the light palette is fully reviewed. Old feature URLs redirect to their panel. See [design decisions and verification limits](UI-DESIGN.md). Public Appllama skills informed the rework; MCP access was unavailable.
...
- **676 frontend unit tests (57 files) and 88 Workers tests pass locally (2026-09-17); 664 across 56 files before the Today rebuild**, covering the Track engine, the unified To-Do merge, space search, the context envelope, the rolling-summary policy and the “did you mean” gate; the five-tab shell and no-dead-link guarantees are pinned by `__tests__/shell-ux.test.ts`, and the renamed nav/panel strings were updated deliberately in `ui-clarity`, `product-flow`, `settings` and `refinement`. The new browser specs ran in CI (no browser binaries here). The first run failed nine specs: four were genuine defects — the five-tab bar used `Link prefetch={false}`, so leaving Today kept the third-party provider script and a pending microphone prompt behind; `?panel=constructor` found `Object.prototype.constructor`; a retired `?panel=track` URL reported “not found” instead of redirecting to the tab; and the unified To-Do opened on a dated view that hid an undated task. All four are fixed with source-level guards, the “expenses findable in under ten seconds on a 390 px viewport” bar now **passes in CI**, and the remaining five specs were corrected to assert what the app promises (per-test context, real labels, real history behaviour). The third run left one failure — finishing a task removed it from “All open” while `groupTodo` skipped done rows, so the Done view listed nothing; that is fixed with a unit guard. The browser suite then reached **70/70 in CI, including the “expense under ten seconds on a 390 px viewport” acceptance bar**; every one of the seven defects it found was invisible to unit tests. The compatibility matrix then failed on WebKit only, and it was right: the light (beta) palette had swapped text tokens on the root element while product.css paints Today and most of Your space with hard-coded dark values, so light-mode text landed on dark panels (3.4:1 and 4.2:1 at 12px — a real AA failure). Tokens are now declared per subtree root whose background flips with them, and the matrix prints axe’s measured reason rather than only the rule id (the `#eabb76 at half opacity` report turned out to be axe sampling the panel mid-`panel-enter`, so the suite settles animations before measuring). The matrix passed on `7b7ec50`, and the full browser suite plus the three-browser matrix then passed on the Today rebuild (CI run `35187910944`: 70/70 e2e, then the compatibility matrix, which had been skipped while the suite was red). Two spec-only changes came after that run and are proven by CI, not by this sandbox (which has no browser): `test.slow()` on the two specs above, and an explicit wait for the activity-log undo to finish its write before `workspace.spec.ts` navigates back to Today — `page.goto` is a real navigation, so it could tear the write down, which is why that spec passed and failed on alternate runs. Today’s capture-review scan then failed once on desktop WebKit and named eleven dialog selectors **without a measured reason**, because that spec reported axe rule ids only; it now shares the matrix’s settle-then-measure helper and prints the colours axe compared, so a repeat would be diagnosable from the annotation. **CI run `35196295432` (head `b339c14`) is green for all four jobs — 70/70 e2e plus the Chromium/Firefox/WebKit × desktop/phone/tablet matrix — and that is the head being merged.** No physical-device run is part of that claim. Two of those specs — the cached-profile test in `settings.spec.ts` and the proactive-voice hour in `workspace.spec.ts` — are marked `test.slow()`: at 45s they were losing a wall-clock race against the runner's own cold-start IndexedDB work, not an assertion, and that is written down here so the timeout is not mistaken for a fixed defect. Nothing was skipped, retried away or loosened. The earlier run of this date recorded **553 frontend unit tests**, including the new expense-recall, fitness-context and brain-provider suites on top of the media-store, media-wiring, entitlements (worker + client) and UI-clarity suites; the Next production build, frontend/Workers TypeScript checks and ESLint are clean. Browser and compatibility counts above are from the last CI run and were **not re-executed** in this sandbox (no browser binaries, no network egress); they must be re-run in CI before release. The compatibility matrix covers Chromium, Firefox and WebKit with desktop/phone/tablet profiles, not physical-device certification. Desktop/mobile Operations and vault screenshots were reviewed with no page errors. Browser flows include explicit pause/resume, shared voice records/reminder drafts, persisted financial edits, approval-to-inbox delivery, reviewed linked import, mobile errors and vault isolation.
...
| 167 | Automated logic/reliability tests | Partial — 676 frontend and 88 Workers tests (2026-09-17) plus the existing browser flows; the new Track/To-Do/search/intent suites are unit-level only and their Playwright specs await a first CI run; media-source health, provider and device matrices outstanding |
