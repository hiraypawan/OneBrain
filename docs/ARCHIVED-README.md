# Historical README — not current setup or verified capabilities

Archived 2026-09-11. These claims, URLs, provider settings and UI instructions describe older builds and must not be used as current product evidence. In particular, paid host-key fallbacks are retired, device behavior is not universally certified, and the current UI has only Today and Your space. Use the root README and current audit instead.
