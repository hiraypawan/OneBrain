...
- **Undo stays next to the action.** The receipt log is in Your space, but a receipt written while Memory is off exists only in the current document, and a panel is a different document. Rather than pretend the log covers it, Today shows one line — last action, `Verified locally`/`Session only`, and Undo — which is also the only honest place for it: it disappears on navigation, exactly like the session it describes.
