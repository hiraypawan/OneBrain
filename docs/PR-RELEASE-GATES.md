...
## Gates added by the 2026-09-15 entitlements and media work

- [ ] **Maintainer sign-off on reversing ledger row 170** ("remove entertainment from core"). Music/podcast playback is back as an opt-in panel; if that decision is rejected, revert `store/media.ts`, `components/MediaPlayer.tsx`, the `layout.tsx` mount, the `useAssistant` media block and the catalog entry rather than shipping it silently.
- [ ] **Probe every keyless media source from the deployed workerd runtime**, not from a laptop or CI runner: JioSaavn search, the Apple podcast directory plus at least one show RSS feed, and each configured Invidious instance. The build sandbox had no network egress, so source health is unverified; record responses, status codes and latency, and remove or replace any source that fails. Do not claim working playback until this passes.
- [ ] **Confirm autoplay behaviour on real iOS Safari, Android Chrome and a Bluetooth earbud pairing**: the player must announce a blocked start ("tap play") instead of appearing to play silence.
- [ ] Apply migration **0008** to the production D1 database in order, after a backup, and confirm `entitlements`, `entitlement_keys` and `entitlement_usage` exist before deploying the worker that reads them.
- [ ] Run `scripts/mint-entitlement-keys.mjs` in a private operator environment, store the plaintext keys outside the repository, and verify a redeemed key changes the plan on a second device for the same account. Plaintext keys must never be committed, logged or pasted into an issue.
- [ ] Decide the legacy-browser-key posture explicitly: `ENTITLEMENTS_ACCEPT_LEGACY_BETA` defaults to rejecting pre-0008 browser unlocks. If it is enabled anywhere, record why and for how long.
- [ ] Re-run the browser and compatibility suites in CI (they were not executed in the build sandbox) and confirm the pinned strings in `frontend/__tests__/ui-clarity.test.ts` still match the rendered UI.
