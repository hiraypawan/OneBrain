...
- **You (`/you`):** account, plan (with who decided it), how OneBrain talks, what it may keep, your Track goals and the AI-engine status, each row linking to the panel that owns the setting — this page never edits anything itself. The previously orphaned `theme` preference is now wired (dark stays the reviewed default; light is a token swap labelled beta).
