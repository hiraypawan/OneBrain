...
- **Track (`/track`):** one tab over the log the app already keeps — 💰 Expenses, 🍎 Food diary, ❤️ Health, 💪 Workouts, each switchable by day, week or month with the window and lens kept in the URL (`/track?lens=expenses&range=month&day=…`), so a spoken answer can hand you a deep link and Back still works. Expenses get per-item rows, category totals read from what you typed or said (never guessed), a monthly “₹X of ₹Y” limit you can set by voice (“set my monthly budget to 20000”), and a CSV export of exactly the rows in view. Food figures are labelled `≈` and carry “not medical advice”; empty windows print ₹0 and say nothing was logged instead of hiding. The raw timeline and manual entry stay in Your space → Fitness log.
- **Voice (`/voice`):** the full-screen listening surface — one large control, live “Hearing: …” captions, the last answer with its provider badge, retry and microphone-denied states, Silent Mode, and the full list of phrases the app understands. Nothing is recorded before you press start.
- **You (`/you`):** account, plan (with who decided it), how OneBrain talks, what it may keep, your Track goals and the AI-engine status, each row linking to the panel that owns the setting — this page never edits anything itself. The `theme` preference used to have no control and no consumer; it now has both — Your space → Advanced offers **Dark** or **Light (beta)**, which swaps tokens on the shared surfaces while dark stays the reviewed default.
- **Your space (`/control`), four sections:** Get things done (one unified **To-Do** merging canvas tasks, reminders and — on request — shared server records, with due dates, priorities and completion written back to the origin, no fourth store), Your record (Track’s log, conversations, memory, drafts, stories), Utilities (tools, vault, music, export/delete, diagnostics) and Make it yours (plan, account, voice, privacy, advanced). One search box covers saved notes, tasks, decisions, conversations, stories, drafts and the log; vault contents are never searched. Retired panel URLs (`?panel=track`, `notes`, `canvas`, `to-do`, `shared-space`) redirect instead of dead-ending.
- **Your space, Connected work panel:** server accounts, shared workspaces and roles, linked records/projects/financial logs, reviewed imports, connector administration, exact approval hashes, leased scheduling, durable receipts, inbox and audits.
...
- **Your space, Music & podcasts panel:** say “play kesariya”, “gaana band”, “next song” or search by text. A single sticky player is mounted once in the root layout, driven by one store, and playback uses keyless community sources (JioSaavn search, Apple podcast directory plus each show's RSS, public Invidious instances). Failures auto-advance and are spoken plainly, with a YouTube search hand-off when nothing streams. **No source could be reached from the build sandbox, so live playback is unproven**; it is opt-in through `settings.musicEnabled`.
- **Your space, Plan panel:** states the plan, **who decided it** (your account on the server, this browser only, or nothing redeemed), whether it was verified this session, and per-feature usage against the limits the server actually enforces. Redeeming a key while signed in applies account-wide; signed out it unlocks that browser only and says so. If the server is unreachable the last confirmed plan is kept for up to 24 hours and labelled unverified. **No payment, invoice, subscription or renewal exists anywhere in the product.**
...
- Local 2026-09-15 run: **527 frontend unit tests, 88 Workers tests, 8 capacity-model tests, Next 15.5.25 production build, TypeScript and ESLint clean.** The browser and compatibility suites above were **not re-run** in that sandbox (no browser binaries, no network egress); CI must confirm them before merge.
- Local 2026-09-16 (later) run: **664 frontend unit tests across 56 files** — the 553 above plus new `track`, `todo-unified`, `space-search`, `context-envelope`/`intent-recovery` and `shell-ux` suites — with the Next 15.5.25 production build, `tsc --noEmit` and ESLint clean. Shipped in this pass: the Track tab, the unified To-Do and consolidated Your space, the five-tab shell with `/voice` and `/you`, a token-budgeted AI context envelope, a rolling conversation summary replacing the every-20-message cadence, “did you mean…?” intent recovery, and voice control of the monthly budget. No browser binaries exist here, so the Playwright suite ran in CI on this branch. Its first run failed nine specs, and four of those were real defects, now fixed: the new tab bar navigated through the client so leaving Today kept the provider script alive; `?panel=constructor` resolved through `Object.prototype`; a retired `?panel=track` URL said “not found” instead of redirecting; and the unified To-Do opened on a dated view that hid a task with no due date. The other five were the specs asserting more than the app promises and have been rewritten to assert what it does. The second run left 2 failures (the canvas store was never loaded on Your space’s document, and exports were named after relative labels), the third left 1 (finishing a task hid it from the Done view too), and each was a real defect, now fixed and pinned by a unit test. **CI is now 70/70 on the browser suite, including the “expense findable in under ten seconds on 390 px viewport” acceptance bar.** The compatibility matrix then failed on WebKit only: the light palette had redefined text tokens on the root element while Today and Your space paint hard-coded dark backgrounds, so light-mode text landed on dark panels and axe reported a real color-contrast violation. The swap is scoped to the surfaces reviewed with it (tab bar, Track, Voice, You), the matrix now prints axe’s measured reason instead of just the rule id, and a re-run must be green before merge, and the Today screen itself was **not** rebuilt (only its navigation was consolidated).
- Local 2026-09-16 run: **553 frontend unit tests (incl. new expense-recall, fitness-context and brain-provider suites), 88 Workers tests, Next 15.5.25 production build, TypeScript and ESLint clean.** Browser and compatibility suites were **not re-run** locally; CI must confirm them before merge. Shipped: deterministic voice answers for logged expenses/food over date ranges ("what expenses did I do yesterday"), fitness-log injection into AI context, live "Hearing: …" captions, honest per-answer provider badges with an offline Gemini-key nudge, and expense discoverability in Your space search.
...
See [the capacity report and calculator](docs/FREE-TIER-CAPACITY.md) for the private service binding, native rate counters, paginated/lazy server data, indexed queries, five-minute bounded scheduler and quota-pause controls. These are real optimizations, **not a guarantee of 10,000 concurrent cloud-active users or unlimited free usage**. The single free D1 database is capped at 500 MB. The entitlement ledger raises the modeled estimate to **70,400 of 80,000** operating written rows/day, so write headroom is thin and any increase in gated usage, scheduler batches or cleanup volume must be re-budgeted. No deployment was performed.
